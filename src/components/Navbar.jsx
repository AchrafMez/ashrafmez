...
function Navbar({ theme, toggleTheme }) {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50">
      <div className="max-w-5xl w-11/12 mx-auto flex justify-between items-center py-2.5 backdrop-blur-md bg-white/80 dark:bg-stone-900/80 border-b border-stone-200/30 dark:border-stone-700/30">
        ...
        <button
          type="button"
          onClick={toggleTheme}
          className="relative w-14 h-7 rounded-full bg-stone-200 dark:bg-stone-700 transition-colors duration-300 focus:outline-none"
          aria-label="Toggle theme"
        >
          <span
            className={`absolute top-0.5 left-0.5 w-6 h-6 rounded-full bg-white dark:bg-stone-900 shadow-sm flex items-center justify-center transition-transform duration-300 ${
              theme === "dark" ? "translate-x-7" : "translate-x-0"
            }`}
          >
            {theme === "dark" ? (
              <svg className="w-3.5 h-3.5 text-yellow-400" viewBox="0 0 24 24" fill="none" stroke="currentColor">
              ...
              <svg className="w-3.5 h-3.5 text-stone-600" viewBox="0 0 24 24" fill="none" stroke="currentColor">
      ...
      {menuOpen && (
        <div className="sm:hidden backdrop-blur-md bg-white/90 dark:bg-stone-900/90 border-b border-stone-200/30 dark:border-stone-700/30">
